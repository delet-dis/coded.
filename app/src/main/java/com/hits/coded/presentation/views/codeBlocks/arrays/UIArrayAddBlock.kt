...
import com.hits.coded.databinding.ViewArrayAddBlockBinding

class UIArrayAddBlock @JvmOverloads constructor(
    context: Context,
    ...
    private val binding: ViewArrayAddBlockBinding

    override val nestedUIBlocks: ArrayList<View?> = ArrayList()

    private var _block = ArrayBlock(ArrayBlockType.PUSH)
    ...
    init {
        inflate(
            context,
            R.layout.view_array_add_block,
            this
        ).also { view ->
            binding = ViewArrayAddBlockBinding.bind(view)
        }
    ...
    private fun handleDropEvent(
        ...
        if (draggableItem != this@UIArrayAddBlock) {
            scaleMinusAnimation(binding.firstCard)
    ...
    private companion object {
        const val DRAG_AND_DROP_TAG = "ARRAY_ADD_BLOCK_"
    }
}